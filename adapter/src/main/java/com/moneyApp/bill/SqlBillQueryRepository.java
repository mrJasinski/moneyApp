...
interface SqlBillQueryRepository extends BillQueryRepository, JpaRepository<BillSnapshot, Long>
{
    ...
    @Override
    @Query(value = "SELECT b.budgetPosition.id AS budgetPositionId, SUM(b.amount) AS sum " +
                   "FROM BillPositionSnapshot b " +
                   "WHERE b.id IN :billPosIds " +
                   "GROUP BY b.budgetPosition.id")
    ...
    @Override
    @Query(value = "SELECT bp.id " +
                   "FROM BillPositionSnapshot bp " +
                   "WHERE bp.budgetPosition.id = :budPosId")
